...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    /// 监听方法, 输入中文也可获得位数
    [QNWSNotificationCenter addObserver:self selector:@selector(textFiledEditChanged:) name:@"UITextFieldTextDidChangeNotification" object:nil];
...
-(void)viewDidDisappear:(BOOL)animated
{
    ...
    [QNWSNotificationCenter removeObserver:self name:@"UITextFieldTextDidChangeNotification" object:nil];
...
-(void)createUI
{
    ...
    if (_array_StrReplace.count > 0) {
        WX_EditingShootingModel *model_Editing = _array_StrReplace.firstObject;
        _top_Label_ReplaceStr.font = [UIFont fontWithName:@"Helvetica-Bold" size:26];

        if (model_Editing.font_Color.length>0) {
//            _top_Label_ReplaceStr.textColor = [WX_ToolClass colorWithHexString:model_Editing.font_Color];
    ...
    CGSize size_StrNum = [WX_ToolClass changeSizeWithString:str_Num FontOfSize:10 bold:ENUM_NormalSystem];
...
-(void)receiveEditingMediaWithArray:(NSMutableArray *)array
{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        if (array_Parameter.count>=8)model_Editing.font_Color = [NSString stringWithFormat:@"0x%@",array_Parameter[7]];
        if (i ==0 && _top_Label_ReplaceStr && _top_Label_ReplaceStr.text.length>0 && model_Editing.font_Color.length>0) {
//            _top_Label_ReplaceStr.textColor = [WX_ToolClass colorWithHexString:model_Editing.font_Color];
            _top_Label_ReplaceStr.textColor = KUIColorFromRGB(0x000000);
        }
...
//        label.center = CGPointMake(view_Img.center.x+model_Editing.rect_Label.origin.x/2-5, view_Img.center.y +model_Editing.rect_Label.origin.y/2);
        ...
        if (res == 0) {
//            [self.view showAndHideHUDWithTitle: WithState:0];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            /// 通知主线程刷新
//            if(res == 1)[self.view showAndHideHUDWithTitle:@"转码失败" WithState:0];
