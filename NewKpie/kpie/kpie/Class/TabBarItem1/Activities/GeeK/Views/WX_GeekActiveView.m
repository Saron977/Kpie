...
-(void)geekVote
{
    if (![BYC_AccountTool userAccount]) {
        ...
        _model_Geek.str_phoneNum = model.contact;
        ...
        [BYC_HttpServers Get:KQNWS_SaveUserVote parametersWithToken:dic success:^(id responseObject) {
...
-(void)clcikEntryWith:(DrawLottery)lotteryType
{
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"DrawLotteryMediaStop" object:nil];
...
-(void)createOrRemoveNotifiWith:(BOOL)isCreate
{
    if (isCreate) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(shareToDraw) name:@"shareToDraw" object:nil];

        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getGeekModel:) name:@"NSNotification_GeekDrawLottery" object:nil];
        ...
        [[NSNotificationCenter defaultCenter]removeObserver:self name:@"shareToDraw" object:nil];
        
        [[NSNotificationCenter defaultCenter] removeObserver:self name:@"NSNotification_GeekDrawLottery" object:nil];
