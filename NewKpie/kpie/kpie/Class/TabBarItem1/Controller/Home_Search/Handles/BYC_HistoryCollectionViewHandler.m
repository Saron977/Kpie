//
//  BYC_HistoryCollectionViewHandler.m
//  kpie
//
...
@interface BYC_HistoryCollectionViewHandler()<UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@implementation BYC_HistoryCollectionViewHandler
...
//        [self requstNetWork];//后台老新版没改，报错注释掉了。
    }
    return self;
}
...
- (void)initParameters {
    ...
    NSMutableArray *mArr_Keywords = [BYC_HistoryKeywordsHandler getHistoryKeyword];
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if(kind == UICollectionElementKindSectionHeader) {
        ...
        else if (indexPath.section == 0 && _array_Data.count == 1 && [BYC_HistoryKeywordsHandler getHistoryKeyword]/*表示是历史数据*/)
...
- (UIView *)customViewForEmptyDataSet:(UIScrollView *)scrollView{
    BYC_ControllerCustomView *emptyView = [[BYC_ControllerCustomView alloc]initWithFrame:CGRectMake(0, KHeightNavigationBar, screenWidth, screenHeight - KHeightNavigationBar) andNotificationObject:self];
