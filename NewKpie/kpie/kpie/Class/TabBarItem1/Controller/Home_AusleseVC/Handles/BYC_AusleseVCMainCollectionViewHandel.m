...
#import "BYC_AusleseVCMainCollectionViewHandel.h"
...
@interface BYC_AusleseVCMainCollectionViewHandel ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
...
@implementation BYC_AusleseVCMainCollectionViewHandel

#pragma mark - 本类系统相关方法(包括自定义的初始化方法)

+ (instancetype)initAusleseVCMainCollectionViewHandel:(BYC_AusleseViewController *)vc {

    BYC_AusleseVCMainCollectionViewHandel *my_Self = [[BYC_AusleseVCMainCollectionViewHandel alloc] init];;
    if (my_Self) {
...
- (void)requestData {
    ...
    @try {
        ...
        if (_models.arr_VideoModels.count == 0) mDic_Param = [@{key_IsUpType:@0} mutableCopy];//表示刷新之前没有视频数据,那就走正常逻辑去请求数据
            ...
            [mDic_Param setObject:_collectionView.header.isRefreshing ? @0 : @1 forKey:key_IsUpType];
            [mDic_Param setObject:model_Video.onOffTime forKey:@"onofftime"];
            [mDic_Param setObject:model_Video.videoID forKey:key_IsVideoId];
        }
    ...
    [BYC_HttpServers requestHomeVCAusleseDataWithParameters:dic_Param success:^(AFHTTPRequestOperation *operation, BYC_AusleseVCModels *models) {
        ...
        if (isVideoId) {//上下拉
            ...
            for (BYC_HomeViewControllerModel *videoModel1 in models.arr_VideoModels) {//上下拉排重
                ...
                for (BYC_HomeViewControllerModel *videoModel2 in _models.arr_VideoModels) if ([videoModel1.videoID isEqualToString:videoModel2.videoID]) isRepeat = YES;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) return CGSizeMake(screenWidth, KWidthOnTheBasisOfIPhone6Size(152));
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        ...
        BYC_HomeViewControllerModel *model = _models.arr_VideoModels[indexPath.row];
        if (model.isVR ==1) videoVC.isVR = YES;
        else videoVC.isVR = NO;
        videoVC.hidesBottomBarWhenPushed = YES;
        [videoVC receiveTheModelWith:_models.arr_VideoModels WithNum:indexPath.row WithType:1];
