...
/**切换的状态*/
@property (nonatomic, assign)  NSUInteger  uInt_SelectType;
...
- (void)initSubViews {
    ...
    _view_NewOrHot = ({
        ...
        UIImageView *imageV = ({
            ...
            imageV.image = [UIImage imageNamed:@"channel_btn_zuixin_n"];
            [view_bg addSubview:imageV];
            imageV.contentMode = UIViewContentModeScaleAspectFit;
            [imageV mas_makeConstraints:^(MASConstraintMaker *make) {(void)make.top.bottom.trailing;}];
            imageV;
        });
        ...
        UILabel *label = ({
            ...
            [view_bg addSubview:label];
            [label mas_makeConstraints:^(MASConstraintMaker *make) {
                
                (void)make.leading.top.bottom;
                make.trailing.equalTo(imageV.mas_leading).offset(-2);
            }];
        ...
        [_button mas_makeConstraints:^(MASConstraintMaker *make) {(void)make.top.leading.bottom.trailing;}];
...
-(void)setArr_ColumnModels:(NSArray<BYC_BaseChannelColumnModel *> *)arr_ColumnModels {
    ...
    _arr_ColumnModels = arr_ColumnModels;
...
-(void)setArr_GroupModels:(NSArray<BYC_BaseChannelGroupModel *> *)arr_GroupModels {
    ...
    ((UILabel *)[_view_NewOrHot viewWithTag:101]).text = _arr_GroupModels[_type].groupname;
}

- (void)buttonAction:(UIButton *)button {
  
    ++_uInt_SelectType;
    if (_uInt_SelectType>=_arr_GroupModels.count) _uInt_SelectType = 0;
    self.type = _uInt_SelectType;
}

-(void)setType:(NSUInteger)type {

    _type = type;
    _uInt_SelectType = _type;
    ((UILabel *)[_view_NewOrHot viewWithTag:101]).text = _arr_GroupModels[_type].groupname;
    if (_delegate && [_delegate respondsToSelector:@selector(scrollSubtitleView:didSelectType:)])
        [_delegate scrollSubtitleView:self didSelectType:type];
}

#pragma Mark - collection的DataSource 和 Delegate方法
-(NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    
    return self.arr_ColumnModels.count;
}
...
-(UICollectionViewCell*)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.str_Title = _arr_ColumnModels[indexPath.item].columnname;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *str = _arr_ColumnModels[indexPath.item].columnname;
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (int i = 0; i < _arr_ColumnModels.count; i++) {
