...
-(instancetype)initWithObjects_safe:(id *)objects forKeys:(id *)keys count:(NSUInteger)cnt {
    NSUInteger newCnt = 0;
    for (NSUInteger i = 0; i < cnt; i++) {
        
        if (!(keys[i] && objects[i])) {
//            NSMutableDictionary *m_Arr = [self mutableCopy];
//            QNWSLog(@"%@",self);
//            QNWSLog(@"(keys[i] == %@  objects[i] == %@",keys[i],objects[i]);
            break;
        }
        newCnt++;
    }
