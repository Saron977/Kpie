...
#define FONT_TOPIC_LIGHT(fontsize) [UIFont fontWithName:@"FZLTHK--GBK1-0" size:(fontsize)]
...
- (UIImageView *)toolbarBackgroundImageView
{
    if (_toolbarBackgroundImageView == nil) {
        ...
        _toolbarBackgroundImageView.backgroundColor = KUIColorFromRGB(0x191e25);
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    if([self stringContainsEmoji:text]){
        [self showAndHideHUDWithTitle:@"暂不支持emoji~" WithState:BYC_MBProgressHUDHideProgress];
//        return NO;
    }else{
...
- (void)textViewDidChange:(UITextView *)textView
{
    if([self stringContainsEmoji:textView.text]){
        [self showAndHideHUDWithTitle:@"不暂不支持emoji~" WithState:BYC_MBProgressHUDHideProgress];
    }else{
...
- (void)setupSubviews
{
    ...
    [self.styleChangeButton setImage:[UIImage imageNamed:@"icon-yuyin"] forState:UIControlStateNormal];
    [self.styleChangeButton setImage:[UIImage imageNamed:@"icon-yuyin"] forState:UIControlStateHighlighted];
    [self.styleChangeButton setImage:[UIImage imageNamed:@"icon-jianpan"] forState:UIControlStateSelected];
...
//    [self.moreButton setImage:[UIImage imageNamed:@"输入框-更多"] forState:UIControlStateNormal];
//    [self.moreButton setImage:[UIImage imageNamed:@"输入框-更多按下"] forState:UIControlStateHighlighted];
//    [self.moreButton setImage:[UIImage imageNamed:@"输入框-键盘"] forState:UIControlStateSelected];
    [self.moreButton setTitle:@"发送" forState:UIControlStateNormal];
    self.moreButton.titleLabel.font = [UIFont systemFontOfSize:15];
    [self.moreButton setTitleColor:KUIColorFromRGB(0xffffff) forState:UIControlStateNormal];
...
//    self.moreButton.layer.borderColor = color;
...
//    self.faceButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.moreButton.frame) - kInputTextViewMinHeight - kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"icon-smiley"] forState:UIControlStateNormal];
    [self.faceButton setImage:[UIImage imageNamed:@"icon-smiley"] forState:UIControlStateHighlighted];
    [self.faceButton setImage:[UIImage imageNamed:@"icon-jianpan"] forState:UIControlStateSelected];
    ...
    // 初始化输入框
    self.inputTextView = [[XHMessageTextView  alloc] initWithFrame:CGRectMake(textViewLeftMargin, kVerticalPadding, width +kInputTextViewMinHeight+kInputTextViewMinHeight, kInputTextViewMinHeight)];
    ...
    _inputTextView.font = FONT_TOPIC_LIGHT(17);
    _inputTextView.returnKeyType = UIReturnKeySend;
    _inputTextView.enablesReturnKeyAutomatically = YES; // UITextView内部判断send按钮是否可以用
    _inputTextView.placeHolder = self.placeHolderString;
    ...
    //录制
    self.recordButton = [[UIButton alloc] initWithFrame:CGRectMake(textViewLeftMargin, kVerticalPadding, width +kInputTextViewMinHeight +kInputTextViewMinHeight, kInputTextViewMinHeight)];
    self.recordButton.titleLabel.font = FONT_TOPIC_LIGHT(15);
    [self.recordButton setTitleColor:KUIColorFromRGB(0x565656) forState:UIControlStateNormal];
    [self.recordButton setTitleColor:KUIColorFromRGB(0xffffff) forState:UIControlStateSelected];
    [self.recordButton setTitleColor:KUIColorFromRGB(0xffffff) forState:UIControlStateHighlighted];
#pragma mark -------------- 键盘修改
    [self.recordButton setBackgroundImage:[[UIImage imageNamed:@"img-yuyinBG-n"] stretchableImageWithLeftCapWidth:10 topCapHeight:10] forState:UIControlStateNormal];
    [self.recordButton setBackgroundImage:[[UIImage imageNamed:@"img-yuyinBG-h"] stretchableImageWithLeftCapWidth:10 topCapHeight:10] forState:UIControlStateHighlighted];
...
- (void)willShowKeyboardFromFrame:(CGRect)beginFrame toFrame:(CGRect)toFrame
{
    ...
    self.inputTextView.placeHolder = self.placeHolderString;
}
...
- (void)recordButtonTouchDown
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(didStartRecordingVoiceAction:)]) {
        [_delegate didStartRecordingVoiceAction:nil];
    }
...
- (void)recordButtonTouchUpOutside
{
    ...
    {
        [_delegate didCancelRecordingVoiceAction:nil];
    }
...
- (void)recordButtonTouchUpInside
{
    ...
    {
        [self.delegate didFinishRecoingVoiceAction:nil];
    }
}

- (void)recordDragOutside
{
    [UUProgressHUD changeSubTitle:UUProgressCancle];
    if ([self.delegate respondsToSelector:@selector(didDragOutsideAction:)])
    {
        [self.delegate didDragOutsideAction:nil];
    }
}

- (void)recordDragInside
{
    [UUProgressHUD changeSubTitle:UUProgressRecording];
    if ([self.delegate respondsToSelector:@selector(didDragInsideAction:)])
    {
        [self.delegate didDragInsideAction:nil];
    }
}
